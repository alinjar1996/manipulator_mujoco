...
class PointNet(nn.Module):
	def __init__(self, inp_channel=1, hidden_dims = 64, emb_dims=512, output_channels=20):
		super(PointNet, self).__init__()
		self.conv1 = nn.Conv1d(inp_channel, hidden_dims, kernel_size=1, bias=False) # input_channel = 3
...
class RobustScaling:
    def __init__(self, median, iqr, eps=1e-8):
        ...
        # self.median_ = torch.median(input_nn, dim=0).values
        # self.inp_q1 = torch.quantile(input_nn, 0.25, axis=0)
        # self.inp_q3 = torch.quantile(input_nn, 0.75, axis=0)
        # self.inp_iqr_ = self.inp_q3 - self.inp_q1
        # # Handle constant features
        # self.inp_iqr_ = torch.where(self.inp_iqr_ == 0, torch.tensor(1.0), self.inp_iqr_)
...
class DataEncoder(nn.Module):
    ...
    Handles:
    - Initial and final Joint Positions (scalar values)
    ...
    def __init__(self, m_dim, k_dim, encoding_dim=100):
        ...
        Args:
            m_dim: Dimension of covariance matrix
...
class Flow(torch.nn.Module):
    def __init__(self, out_chan, th_i_m, th_i_s, th_f_m, th_f_s, x_f_m, x_f_s, q_f_m, q_f_s, cov_m, cov_s): 
    ...
    def forward(self, x_t: Tensor, terrain_data: list, t: Tensor, pcd: Tensor) -> Tensor:
        ...
        target_pos = self.target_pos_norm.normalize(terrain_data[2])
        target_orientation = self.target_orientation_norm.normalize(terrain_data[3])
        ...
        cond = self.terrain_encoder(theta_init, theta_fin, target_pos, target_orientation)
