...
class cem_planner():
	...
	def __init__(self, num_dof=None, num_batch=None, num_steps=None, timestep=None, maxiter_cem=None, num_elite=None, w_pos=None, w_rot=None, w_col=None, 
		...
		# Batching over axis 0 of init_qpos, target_pos, and target_quat
		self.batched_solve_ik = jax.vmap(self.solve_ik, in_axes=(None, 0, 0, 0))

		self.ik_solver = InverseKinematicsSolver(self.model_path, self.mjx_data.qpos[:self.num_dof])
	...
	def solve_ik(self, model_path, init_qpos, target_pos, target_quat):
		solver = InverseKinematicsSolver(model_path, init_qpos)
		solver.set_target(target_pos, target_quat)
		ik_vel = solver.solve(dt=self.t)

		return ik_vel  # e.g. final joint velocities
	...
	@partial(jax.jit, static_argnums=(0,))
	def compute_cost_single(self, thetadot, eef_pos, eef_rot, collision, target_pos, target_quat):
		cost_g_ = jnp.linalg.norm(eef_pos - target_pos, axis=1)
		cost_g = cost_g_[-1] + jnp.sum(cost_g_[:-1])

		dot_product = jnp.abs(jnp.dot(eef_rot/jnp.linalg.norm(eef_rot, axis=1).reshape(1, self.num).T, target_quat/jnp.linalg.norm(target_quat)))
	...
	def cem_iter(self, carry,  scan_over):
		...
		init_pos, init_vel, target_pos, target_quat, xi_mean, xi_cov, key, state_term, lamda_init, s_init, xi_samples = carry
		...
		#ik_solver = InverseKinematicsSolver(self.model_path, mjx_data.qpos[:num_dof])

		# Prepare batched inputs
		init_qpos_batch = jnp.tile(self.mjx_data.qpos[:self.num_dof], (self.num_batch, 1))         # shape (B, DOF)
		# target_pos_batch = jnp.tile(target_pos, (self.num_batch, 1))  # shape (B, 3)
		# target_quat_batch = jnp.tile(target_quat, (self.num_batch, 1))  # shape (B, 4)

		target_pos_batch = target_pos
		target_quat_batch = target_quat

		print(f"target_pos: {target_pos.shape}")
		print(f"target_quat: {target_quat.shape}")	
		print(f"target_pos_batch: {target_pos_batch.shape}")	
		print(f"target_quat_batch: {target_quat_batch.shape}")	

		# Run bat
		# print	ched IK
		thetadot = self.batched_solve_ik(self.model_path, init_qpos_batch, target_pos_batch, target_quat_batch)
		...
		cost_batch, cost_g_batch, cost_r_batch, cost_c_batch = self.compute_cost_batch(thetadot, eef_pos, eef_rot, collision, target_pos, target_quat)
		...
		carry = (init_pos, init_vel, target_pos, target_quat, xi_mean, xi_cov, key, state_term, lamda_init, s_init, xi_samples_new)
	...
	def compute_cem(
		...
		target_quat = jnp.tile(target_quat, (self.num_batch, 1))
		...
		carry = (init_pos, init_vel, target_pos, target_quat, xi_mean, xi_cov, key, state_term, lamda_init, s_init, xi_samples)
...
def main():
	...
	target_quat = jnp.array([0.0, 0.0, 0.0, 1.0])
